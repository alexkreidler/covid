...
In this article, I firstly describe the issues with our current testing method,
...
100 nl of samples, and would require many in parallel to gain throughput
capabilities. [@neuzil_ultra_2006]
...
This is why LAMP tests have become popular as more accurate form of rapid
...
develop that could test individual dead mosquitoes by simply placing them in a
...
a swab down one's nasal cavity, often elicitng coughs or sneezes that could
contain the virus. This is why healthcare workers need to wear and change into
new personal protective equiment (PPE) before every test.

This leads to real challenges, including the simple time added for healthcare
workers to properly clean or change their PPE before a test, and in situations
where there are supply chain issues resulting in a lack of PPE for doctors and
others who need it.
...
respiratory diseases, all indications are that patient-collected samples using a
nasal swab (which is much easier and less uncomfortable) or saliva are just as
accurate as nasopharyngeal swabs.
...
may test negative multiple times, but still be clincally diagnosed with COVID-19
for other reasons (blood tests, CT scans, etc). [@hu_clinical_2020]
...
clinically diagnosed as having COVID. Thus, CT scans are highyl recommended for
more comprehensive diagnosis in endemic areas. [@ai_correlation_2020]
...
It be used for complex biological procedures such as "genotyping, cloning,
mutation detection, sequencing, microarrays, forensics, and paternity testing."

However, for simple diagnostic testing, where the basic goal is to amplify the
RNA (in this case) such that it can be detected by a flourescent gel or a change
in pH, but with some constraints, it seems that using the best tool for the job,
even if it isn't widely used in labs, makes the most sense.

Some of these constraints are that it is **specifc** and not "cross-reactive"
(doesn't detect similar viruses like SARS, MERS H1N1, etc) and has a high
**sensitivity** and **low level of detection** (e.g. it can detect the virus if
only 10 genome equivalent copies of RNA are present in a sample)
...
if they have the capability to improve their capacity by deploying RT-LAMP in
some way.
...
Firstly, we would want to enable patients to collect their own samples in their
homes. This would allow for patients to preemptively self-quarantine, recieve
the sampling tools in the mail and perform their own test.
...
This would allow them to run the test for themselves as soon as they recieve the
...
The tests and testing procedures would have to recieve and Emergency Use
Authorization (EUA) from the FDA.

All laboratories processing human samples are required to be follow the federal
statute Clinical Laboratory Improvement Amendments (CLIA).

However, for tests which are simple, use unprocessed specimens (like saliva or
...
National Guard personell to operate the facilities.
...
However, there are reports of one town in Italy which tested all of its
citizens, isolating the positive cases and then retesting periodically. The
virus in that town ran its course and died out.

Another possibility is to test as many people possible in the United States. One
idea for encouraging this is to tie the cash check in proposed economic stimulus
package to completing a self-test. This would then ensure with multiple rounds
of data whether or not every American has the virus.
...
have drawn conclusions from this research is: Why does our developed health-care
system feel the need to stick with this type of test when there is a better
option out there?

In fact, this is the question I posed to David Walt and Pardis Sabeti of Harvard
and the Greater Boston Consortium on Pathogen Readiness.
